...
namespace MS_lab8
{
    public class T
    {
        ...
        {
            bool f = true;
            List<string> fromPositionsNames = arcsIn.Select(x => x.placeFrom.name).ToList();
            ...
            {
                Console.WriteLine(connectedPositions[i].name);
                Console.WriteLine(connectedPositions[i].markersCount);
                Console.WriteLine("mol");
                Console.WriteLine(arcsIn[i].name);
                Console.WriteLine(arcsIn[i].n);
        ...
        public List<P> performTransaction(List<P> positions/*, bool ver*/)
        {
            ...
            {
                positions.Where(x => x.name == a.placeFrom.name).FirstOrDefault().markersCount -= a.n;
                Console.Write("{0} ", a.placeFrom.name);
            }
            Console.WriteLine();
